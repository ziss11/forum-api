...
class ThreadsHandler {
    constructor(container) {
        ...
        this.postThreadCommentsHandler =
            this.postThreadCommentsHandler.bind(this);
        this.getThreadByIdHandler = this.getThreadByIdHandler.bind(this);
        this.deleteThreadCommentsHandler =
            this.deleteThreadCommentsHandler.bind(this);
        this.postCommentsReplyHandler =
            this.postCommentsReplyHandler.bind(this);
        this.deleteCommentsReplyHandler =
            this.deleteCommentsReplyHandler.bind(this);
    }
    ...
    async deleteThreadCommentsHandler(request, h) {
        const deleteCommentsUseCase = this._container.getInstance(
            DeleteCommentsUseCase.name
        );
    ...
    async postCommentsReplyHandler(request, h) {
        const addCommentsReplyUseCase = this._container.getInstance(
            AddCommentsReplyUseCase.name
        );
        ...
        const addedReply = await addCommentsReplyUseCase.execute({
            owner,
            threadId,
            commentId,
            content,
    ...
    async deleteCommentsReplyHandler(request, h) {
        const deleteCommentsReplyUseCase = this._container.getInstance(
            DeleteCommentsReplyUseCase.name
        );

        const { id: owner } = request.auth.credentials;
        const { threadId, commentId, replyId } = request.params;

        await deleteCommentsReplyUseCase.execute(
            owner,
            threadId,
            commentId,
            replyId
